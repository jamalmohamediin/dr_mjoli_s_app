...
export const generateSinglePagePDF = async (
  patientName: string,
  patientId: string,
  diagrams?: NewDiagramCapture[],
  reportData?: any
) => {
  try {
    console.log('=== GENERATING NEW SINGLE PAGE PDF ===');
    ...
    const availableHeight = pageHeight - footerHeight - margin;
    
    let y = margin;
    
    // HEADER - Matching live report style
    pdf.setFontSize(16);
    ...
    if (reportData?.selectedProcedures && reportData.selectedProcedures.length > 0) {
      ...
      // Group procedures into 3 columns
    ...
    // Separator line before diagrams
    pdf.setDrawColor(128, 128, 128);
    pdf.line(margin, y, pageWidth - margin, y);
    y += 8;
    
    // TABLE-STYLE LAYOUT WITH SIDE-BY-SIDE DIAGRAMS
    const gastro = diagrams?.find(d => d.type === 'gastroscopy');
    const colono = diagrams?.find(d => d.type === 'colonoscopy');
    
    if (gastro || colono) {
      // Calculate optimal sizing for single page fit
      ...
      const remainingHeight = availableHeight - (y - margin);
      const headerHeight = 8;
      const imageHeight = Math.min(85, (remainingHeight - headerHeight - 40) * 0.6); // 60% for images, 40% for text
      const findingsHeight = remainingHeight - headerHeight - imageHeight - 10;
      
      // TABLE HEADERS - matching live report style (no visible borders)
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'bold');
      ...
      // FINDINGS SECTIONS (no visible borders)
      const findingsBoxHeight = Math.max(findingsHeight, 40);
      
      // Set font for findings - matching live report style
      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(0, 0, 0);
      ...
      if (gastro && reportData?.gastroscopyFindings?.findings?.length > 0) {
        ...
        reportData.gastroscopyFindings.findings.forEach((finding: any, index: number) => {
          if (leftY > maxFindingsY - 8) return; // Stop if running out of space
      ...
      if (colono && reportData?.colonoscopyFindings?.findings?.length > 0) {
        ...
        reportData.colonoscopyFindings.findings.forEach((finding: any, index: number) => {
          if (rightY > maxFindingsY - 8) return; // Stop if running out of space
    ...
    // Add conclusion if available and space permits - matching live report style
    ...
    // Add follow-up information if available and space permits - matching live report style
    ...
    const filename = `NEW_Endoscopy_Report_${timestamp}.pdf`;
    ...
    console.error('Error in new PDF generator:', error);
    throw error;
  }
};